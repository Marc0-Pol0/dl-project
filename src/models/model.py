...
class StockLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # The output does NOT include Softmax here: we do it in the loss function 
        return out
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        # positional encodings computed once in log space
...
class StockTransformer(nn.Module):
    def __init__(
        self, 
        input_size: int,
        d_model: int,  # model's internal representation dimension
        nhead: int,
        num_encoder_layers: int, 
        dim_feedforward: int,  # FFN hidden layer dimension
        output_size: int,
        dropout: float = 0.1,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # We extract the representation for the LAST element (Day 30)
        final_state = out[:, -1, :] 
