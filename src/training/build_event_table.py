...
- final_data_500.pkl:
    dict[ticker] -> DataFrame indexed by trading-day DatetimeIndex (date-level),
    with columns like positive/negative/neutral + fundamentals + adj_price + MA3/MA6.
- earning_dates_500.pkl:
    dict[ticker] -> DataFrame with column "Earnings Date" (tz-aware, America/New_York).

Output:
    One row per earnings event with controlled feature snapshots and leakage-safe sentiment aggregates.
"""
...
def main() -> None:
    ...
    table = build_event_table(
        ...
        cfg=cfg,
        keep_unknown_timing=KEEP_UNKNOWN_TIMING,
    )
